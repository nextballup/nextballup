...
This document archives the detailed audit rationale, longer architecture notes, and future-looking constraints that were split out of `CLAUDE.md` to keep Claude Code startup context lean.

Use this file when working on:

- security/compliance hardening
- worker/CV implementation
- legal/privacy edge cases
- later product phases beyond the backend foundation

For day-to-day implementation guidance, read `README.md` first and `CLAUDE.md` second.
...
- **Stack**: Python 3.12.x pinned (backend uv workspace, CV package isolated for now) · FastAPI · PostgreSQL 16 · Redis · Next.js 15 · Celery
...
- **Repo state**: Spec scaffold first. Many files below describe the target architecture and are not all implemented yet.
...
# Root dev tooling plus the lightweight backend workspace
uv sync
```

Do not assume `alembic/`, `apps/web/`, runtime entrypoints, or tests exist until they are checked in. Keep `packages/cv_pipeline` out of the root uv workspace until the M5 Max arrives and the CV layer is actively being built.
...
- **Multi-tenancy**: Team is the tenant boundary. PostgreSQL row-level security MUST be enabled in the initial Alembic migration for every tenant-scoped table. All database queries on tenant-scoped data MUST also filter by team_id in application code; middleware and SQLAlchemy guards are defense-in-depth, not the primary barrier.
...
jobs:
  python-audit:
    ...
    steps:
      ...
      - run: uv sync
      - run: uv run --no-sync pip-audit --strict  # Fails on known vulnerabilities

  npm-audit:
    if: ${{ hashFiles('apps/web/package.json') != '' && hashFiles('apps/web/pnpm-lock.yaml') != '' }}
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
  ...
  license-check:
    ...
    steps:
      ...
      - uses: astral-sh/setup-uv@v4
      - run: uv sync
      - run: uv run --no-sync pip-licenses --fail-on="GPL;AGPL;SSPL"  # Block copyleft
```
...
-- Row-level security for tenant isolation (baseline control)
-- Applied in the initial migration for every tenant-scoped table:
...
### Layer 5: Data
PostgreSQL 16 (23 tables including audit log), Redis 7 (cache db0, Celery broker db1, Celery results db2), append-only audit log (3-year retention, monthly partitions, immutability triggers), row-level security enabled on tenant-scoped tables from the first migration.

### Layer 6: API (FastAPI)
RS256 JWT auth (15min access, 7d refresh), RBAC (coach/player/admin), tenant isolation at database and query layers, 50+ REST endpoints, WebSocket processing status via **Redis pub/sub message bus** (enables multi-instance API scaling), signed video URLs, rate limiting.
